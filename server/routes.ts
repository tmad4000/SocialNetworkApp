...
export function registerRoutes(app: Express): Server {
  ...
  // Add related posts routes near the posts section
  app.post("/api/posts/:id/related", async (req, res) => {
    ...
    if (isNaN(postId) || !relatedPostId) {
      return res.status(400).send("Invalid post ID or related post ID");
    }
    ...
    try {
      ...
      if (!sourcePost || !relatedPost) {
        return res.status(404).send("One or both posts not found");
      }
      ...
      const [newRelation] = await db
        ...
        .values({
          ...
          similarity: 1, // Direct manual relation has maximum similarity
        })
        .returning();

      res.json(newRelation);
    } catch (error) {
      console.error('Error creating related post relationship:', error);
      res.status(500).send("Error creating related post relationship");
    }
  });
  ...
  app.get("/api/posts/:id/related", async (req, res) => {
    ...
    try {
      // Get the source post with its related posts
      const relatedPostsList = await db.query.relatedPosts.findMany({
        where: eq(relatedPosts.sourcePostId, postId),
        with: {
          relatedPost: {
            with: {
              user: {
      ...
      const transformedPosts = relatedPostsList.map(relation => ({
        ...relation.relatedPost,
        similarity: relation.similarity,
        likeCount: 0, // You might want to add these counts
        liked: false, // These should be computed based on the current user
        starred: false,
        privacy: 'public', // Add proper privacy handling
      }));
